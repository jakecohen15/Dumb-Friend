...
export default function App() {
  ...
  async function createGame() {
    ...
    // Polling fallback: check every 2s if opponent joined (in case realtime misses it)
    const pollId = setInterval(async () => {
      const { data: g } = await supabase.from('games').select('status, player2_id').eq('id', game.id).single()
      if (g && g.status === 'countdown' && g.player2_id) {
    ...
    // Store poll so we can clear it later
  ...
  function goHome() {
    ...
    if (waitRef.current) clearInterval(waitRef.current)
    if (pollRef.current) clearInterval(pollRef.current)
    setScreen('home'); setGameId(null); gameIdRef.current = null; questionsRef.current = []; countdownStartedRef.current = false
  }
